...
from signals_blocks import (
    ...
    # _ticker_block/_confirm_dialog relocated 2026-08-15, once the origin-
    # column fix for bugs #54/#63-64 was confirmed current in this module and
    # carried across intact (an earlier relocation attempt in a parallel
    # worktree was based on a stale pre-fix copy and had to be reverted).
    _confirm_dialog, _ticker_block,
)
...
def _attempt_automated_buy(node, sizing):
    ...
    try:
        ...
        db.log_coverage_event("automated_buy_execution", _coverage_mode(account), ticker=ticker,
...
def _verify_resting_before_replace(pos, node, account, ticker, resting_order_id, label):
    ...
    if pos.get('provenance') == 'manual':
        ...
        _post_message(
            ...
            node_id=node.get('id'), incident=True, pos=pos,
        )
...
def alert_stale_price_exit_suppressed(pos):
    ...
    Checked against coverage_snoozes (2026-08-17): without it a known,
    human-acknowledged persistent stale-data condition had no snooze lever at
    all, so it kept inflating this scenario's daily/all-time counts on the
    accountability grid with a condition someone had already explained. Snoozes
    are time-bounded by design (is_snoozed compares against
    datetime('now','localtime')), so one can't silently persist -- it expires
    and resumes alerting rather than silencing the scenario forever.

    DELIBERATELY DIVERGES from _alert_reconcile_mismatch, which skips its
    coverage_events row entirely while snoozed: there, a snooze acknowledges a
    known FALSE POSITIVE, so suppressing the record loses nothing. Here the
    event records something that genuinely happened -- a real position went
    unmonitored for that poll -- and dropping it would leave only the log_poll
    trace, while also contradicting the unconditional-logging contract stated
    just below. So the row is still written, tagged result='skipped_snoozed'
    so the Grid can exclude it from its counts (scripts/coverage_registry.py's
    bad_results for this row); only the Slack alert is suppressed (both
    reviewers, rebuttal round, 2026-08-17)."""
    ticker = pos['ticker']
    account = pos.get('account')
    _snoozed = db.is_snoozed("stale_price_exit_check_skipped", ticker=ticker,
                              account=account, node_id=pos.get('wl_id'))
    node = db.get_watch_list_node_by_id(pos.get('wl_id'))
    # Logged BEFORE the throttle check, deliberately -- should_alert_live's own
    # docstring requires every caller's coverage_events/log_incident call to
    # stay unconditional so nothing is silently lost, only its real-time Slack
    # visibility. Placing it after the _throttled() early-return would have
    # made the record of a persistent same-day data outage as sparse as the
    # (15-min-cooldown) alert itself. Matches _alert_reconcile_mismatch's own
    # ordering (paired review finding, 2026-08-17).
    db.log_coverage_event("stale_price_exit_check_skipped", _coverage_mode(account), ticker=ticker,
                           position_id=pos.get('id'), node_id=pos.get('wl_id'),
                           result="skipped_snoozed" if _snoozed else "skipped")
    if _snoozed:
        return
    ...
    for pos in open_positions:
        ...
        if last_exc is not None:
            ...
            if _throttled(_RECONCILE_FETCH_FAIL_ALERTED, account, _RECONCILE_COOLDOWN_SECS):
                try:
                    # NOT node_id-gated, deliberately -- this alert is keyed and
                    # throttled per ACCOUNT (a broker-connectivity problem, not a
                    # per-node condition), but should_alert_live is a per-node
                    # decision. Gating this on whichever position happens to be
                    # first in the poll's iteration order would let a sub-threshold
                    # node's failure consume the account's cooldown window and
                    # silently swallow the identical alert for a real capital-at-
                    # stake position on the same account (paired review finding,
                    # 2026-08-17). Left ungated until the throttle key itself is
                    # split per (account, node_id) or per-account capital-at-stake
                    # is computed some other way.
                    _post_message(
...
def _throttled_entry_abandon_alert(wl_id, kind, text):
    ...
    if not _throttled(_ENTRY_ABANDON_ALERTED, f"{wl_id}:{kind}",
        ...
        return
    # NOT node_id-gated, deliberately -- check_entry_abandon's whole design
    # reads account/config from the PINNED pending_buys snapshot (pb['node']),
    # never a live watch_list re-fetch, specifically so a later account/config
    # edit can't silently steer a real cancel_order call at the wrong account
    # (see check_entry_abandon's own docstring). Passing node_id here would
    # have _post_message's gate re-resolve the LIVE node instead, so a
    # real-order-in-flight alert could go silent based on config the resting
    # order predates (paired review finding, 2026-08-17).
    _post_message(text)
    ...
    for pb in db.get_pending_buys():
            ...
            if time.time() - first_seen >= _MARKET_BUY_UNKNOWN_ALERT_AFTER_SECS:
                if _throttled(_MARKET_BUY_UNKNOWN_ALERTED, str(wl_id), _MARKET_BUY_UNKNOWN_ALERT_COOLDOWN_SECS):
                    ...
                    _post_message(f"⚠️ *{ticker}* ({account} · {mode_tag(account, node)}) — market-buy order "
                                  ...
                                  f"order too old to look up) — verify manually at the broker.",
                                  node_id=wl_id, incident=True, node=node, real_order=True)
            continue
        ...
        if executed > 0:
            ...
            _post_message(f"🚨 *{ticker}* ({account} · {mode_tag(account, node)}) — market-buy order "
                          ...
                          f"needed).", node_id=wl_id, incident=True, node=node, real_order=True)
            continue
        ...
        _post_message(f"\U0001F6AB *{ticker}* ({account} · {mode_tag(account, node)}) — market-buy order "
                      ...
                      f"position resulted. Verify at the broker if this was unexpected.",
                      node_id=wl_id, incident=True, node=node, real_order=True)
...
def _fill_dry_run_buy(node, pb, price):
    ...
    # DELIBERATELY INERT: node_id with no incident=True means _post_message's
    # gate falls to should_alert_live. Genuinely unreachable here: this
    # function's only callers (update_dry_run_buys) re-check
    # _effectively_dry_run against a freshly-read node every poll, and
    # effectively_dry_run being False is a precondition of
    # has_capital_at_stake, so should_alert_live can never be True on this
    # path. Never posts, by design (2026-08-08: dry_run chatter must never
    # alert live). The log_slack_message('suppressed') row is the only
    # artifact -- _post_message has NO console branch, so a suppressed
    # message prints nothing anywhere. Do NOT "fix" this into firing by
    # adding incident=True.
...
def check_dry_run_sim_sells(last_seen_bar, dry_run_sell_alerted, load_cache):
    ...
    for pos in db.get_open_positions():
        ...
        if just_activated_trailing:
            ...
            # the NODE's current state -- and the loop above selects on the
            # POSITION's is_dry_run_sim without constraining that. So this is
            # suppressed for every realistic case but is NOT a hard invariant:
            # a canary node later promoted to live at >= the capital-at-stake
            # threshold with a stale sim position still open would make
            # should_alert_live True and post this for real (rebuttal round,
            # 2026-08-17 -- an earlier version of this comment wrongly asserted
            # it could never fire). Intended to stay silent (2026-08-08:
            # dry_run chatter must never alert live). Do NOT "fix" this into
            # firing by adding incident=True.
...
def check_drought_handoff(node):
    ...
    if order_id is None:
        ...
        _post_message(f"⚠️ *{ticker}* ({account} · {mode_tag(account, node)}) — core signal fired again but the "
                      ...
                      node_id=wl_id, incident=True, pos=pos)
        return
...
def _place_stop_loss_for_addon_leg(leg_id, parent_pos, node):
    ...
    try:
        ...
        _post_message(f"🚨 *{ticker}* ({account} · {mode_tag(account, node)}) ADD-ON LEG UNPROTECTED — "
                      ...
                      node_id=parent_pos.get('wl_id'), incident=True, pos=parent_pos)
        return
    ...
    for leg in db.get_open_addon_legs(paper=False):
        ...
        if leg.get('entry_status') == 'placed' and leg.get('entry_order_id'):
            ...
            _post_message(f"⏱️ *{ticker}* ({account} · {mode_tag(account, _leg_node)}) — add-on leg entry order "
                          ...
                          node_id=leg.get('wl_id'), incident=not did_cancel, node=_leg_node)
            continue
    ...
    if leg.get('entry_status') == 'placed':
        ...
        if limits is None:
            ...
            _post_message(f"⚠️ *{ticker}* ({account!r}) — add-on leg ({leg['id']}) core position closed but "
                          ...
                          node_id=leg.get('wl_id'), incident=True, pos=pos)
            return
        if order_id and not _effectively_dry_run(account, node):
            try:
...
def drain_fill_queue(source='daemon'):
    ...
    while True:
        ...
        if side != 'BUY':
            continue
        # `account` here is Schwab's raw ACCT_ACTIVITY AccountNumber (e.g.
        # "45111931"), NOT an alias -- every downstream call in this function
        # (get_filled_order, _coverage_mode, mode_tag, _reconcile_buy_fill)
        # expects the alias schwab_safety.ACCOUNTS/_resolve_account_hashes
        # key on. Found 2026-08-16 (fake-venue harness): passing the raw
        # number straight through raised KeyError inside get_filled_order
        # before anything could reconcile, silently dropping the event (and
        # the rest of that cycle's queue) via this loop's outer try/except.
        # Resolved via the same SCHWAB_ACCOUNT_<ALIAS> suffix match
        # schwab_client._resolve_account_hashes already trusts -- no new
        # lookup table. A resolution failure (env drift, brand-new unlinked
        # account) fails safe to the slow check_auto_fills poll rather than
        # crashing the drain loop for every other queued event this cycle.
        try:
            account = schwab_client.resolve_account_alias_from_number(account)
        except ValueError as e:
            # mode is genuinely unknown here -- the alias resolution that would
            # tell us live vs dry_run/paper is exactly what just failed.
            # "dry_run" would misreport a real live-account failure as
            # low-severity (same reasoning as signals_helpers.mode_tag's
            # UNKNOWN default -- never default an unresolved account to the
            # reassuring label). Found by session-wrap review 2026-08-16.
            db.log_coverage_event("fast_path_fill_reconciliation", "unknown", ticker=ticker,
                                   result="account_number_unresolved", detail=str(e))
            continue
        ...
        if _matching_pending is None and ticker in schwab_safety.AUTOMATION_ENABLED_TICKERS:
            # int-coerced (see _order_id_int above/docs/backlog_cache.md's
            # SchwabOrderID item): the stream's SchwabOrderID is a quoted
            # string ("1007506544737"), Schwab's REST orderId is a number --
            # get_filled_order's `o.get("orderId") == order_id` never coerces,
            # so a raw string order_id here always misses the exact-order
            # lookup and burns every poll attempt before degrading to the
            # slow poll. Not a correctness gap (never falls through to the
            # fuzzy heuristic), just wasted latency -- fixed 2026-08-16
        ...
        fill = None
        for _ in range(_GAP_FILL_POLL_ATTEMPTS):
            fill = schwab_client.get_filled_order(account, ticker, 'BUY', order_id=_order_id_int)
            if fill is not None:
                break
            time.sleep(_GAP_FILL_POLL_INTERVAL_SECS)
        if fill is None:
