...
- **Known bug, `add_node`'s `fixed_sl` computation (2026-07-18, not yet fixed)**: for any `strategies.uses_fixed_sl` strategy, `add_node` always computes `fixed_sl` from `config.json`'s global `execution.fixed_stop_loss` (currently 15%), ignoring whatever real per-node SL value the caller actually wants — no override parameter exists. Silently produced `fixed_sl=15.0` on 19 v4 (SL=1%) nodes inserted this session; worked around by inserting via direct SQL instead of `add_node`. See `docs/backlog_cache.md` for the fix plan.
