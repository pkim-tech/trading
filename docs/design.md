...
- `pending_buys` DB table (added 2026-07-09) — mirrors `trail_state` for the entry side: a trailing-buy order has no open_positions row yet to hang state off of, so this table tracks ticker/node/signal price+time/reminder bookkeeping from `notify_buy_signal` until Executed/Skipped resolves it. `check_buy_reminders()` nags every `BUY_REMINDER_MINUTES` (15) until confirmed, same cadence/supersede pattern as `check_trailing_reminders`.
...
- **Reference report** (`send_reference_report`, renamed from `send_startup_report` 2026-07-09) — fires at startup/restart and at fixed daily times (9:20 AM, 3:20 PM ET), reading off `build_reference_table` (the single computation shared with `_send_window_alert` and `scripts/reference_table.py`). Renders one mrkdwn prose block per ticker (mobile-readable; the old wide code-block table is now CLI-only) split into Open Positions / Buy Candidates sections; dark-theme chart attached only for buy candidates within 5% of trigger. `_send_window_alert` (fires inside the 10:25/15:25 signal windows) reuses the same row data but only shows tickers within 5% of their trigger, not the full watchlist.
