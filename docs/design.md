...
**Old tickers deliberately NOT retired yet** — all 11 keep running in parallel (`ira`, unchanged) until the new FAS/FAZ nodes have organically fired and been confirmed correct, per the project's standing "never force/fake a live test" convention. `scenario_expectations` rows were repointed (old rows deactivated via `active=0`, not deleted; new rows added against the FAS/FAZ `wl_id`s) so the Grid tracks the new nodes going forward, but the old nodes' `watch_list` rows are untouched. Retiring them is a deliberate follow-up, not part of this build. Verification: `signals_invariants.py` clean, full suite 812/812.
