...
**Proposed shape, not yet designed**: since the order quantity is fixed at placement time but a real `TRAILING_STOP` order's linked stop price is continuously live at the broker, either (a) periodically cancel/replace the resting order with a re-sized quantity as the current running-low/trigger updates (needs a poll loop and cancel+replace order calls, not yet built — `schwab_client.py` has no order-modify/cancel wrapper today), or (b) accept the fixed-quantity worst-case sizing but place a *second, top-up* order once the real fill price is known (via the manual Filled confirmation or the new opt-in `check_auto_fills`) to deploy the leftover budgeted capital (worst-case gap + rounding remainder) immediately after the fact. Not scoped — needs design discussion on which approach, and whether the added complexity/order-churn is worth it given `AUTOMATION_ENABLED_TICKERS` is currently just GDXD's $5k pilot book (small dollar impact today, more relevant once the pilot scope widens).
**Action needed**: design session, not started.
