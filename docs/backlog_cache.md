...
## [backtest] Deep dive 2026-08-05 (very late) — drought-overlay: intraday vol entry-gate found (real but ticker-dependent, unresolved overfitting risk), exit-side vol-spike rule tested and rejected (net negative), HANDOFF mechanism traced, per-ticker 5-axis sweep built
Full detail: `docs/research_log.md`'s 2026-08-05 (very late 3) entry. Concentration-risk screen (original 2026-08-05 finding, KORU/pooled-cell dominance) still unresolved — same class of problem now also shows up in the per-ticker sweep (KORU's overlay n=16 outperforms its own 76-trade core strategy, a classic overfit shape). **Deferred to next session, per user's explicit call**: (1) out-of-sample validation of the per-ticker vol-gate choice (fit first-half/test second-half per ticker), (2) whether gate-helped tickers (HIBL/SOXL/UDOW) share a real structural property vs. gate-hurt ones (KORU/DPST/GDXU), (3) per-ticker exit-vol-spike research (today's version was pooled/uniform and found net-negative — a per-ticker treatment untried), (4) HANDOFF/handover mechanics redesign — whether the overlay should transfer management into the core strategy's own state machine instead of sell-then-implicitly-rebuy at the backstop bar's Open, (5) put-hedge cost/payoff modeling (rough Black-Scholes off the same intraday realized-vol measure as an IV proxy) — discussed, not built. Put-hedge option-selection methodology (below) stays blocked on the concentration-risk fix landing first.
