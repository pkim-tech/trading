...
Root cause of the trail_buy_pct/take_profit bug recurring three times today wasn't three independent slips — it's that columns like `take_profit` (means different things per strategy, NULL for TrailingBoth) and `trail_buy_pct` (real for TrailingBoth, always-zero for TrailingExit) have no documented, enforced contract, so every new query has to rediscover the semantics. Real fix: an explicit schema definition (which column means what per strategy, which must always be held fixed together in a neighbor search) future code follows instead of assumes. **User's explicit call: deferred to next phase, tied to whenever a new strategy variant is actually designed** — not tonight, and the still-open 2026-08-02 prune bug (same root cause, `take_profit IN (?)` silently matching nothing for TrailingBoth) stays deliberately unfixed too for now. "Limp along" on the current ad hoc-patch approach until then.

## [live-trading][security] High priority, designed not built, 2026-08-07 — live automation for drought overlay/add-on/put-hedge as generic `watch_list` features
Full design (real schema, state machines, function signatures, truth-table dimensions) + staged pre-work checklist: `docs/design.md`'s 2026-08-07 entry ("Live automation design: drought overlay, margin add-on-at-arm, put-hedge"). The 8-item checklist itself is now a standalone, reusable standard — `docs/new_mechanism_promotion_standard.md`, referenced from `CLAUDE.md`'s Key Files — for any future genuinely-new live-trading mechanism, not just this one. Real new asset class (options order placement, never done in this system before) + two new position-tracking concepts (drought-overlay positions, linked add-on legs). Explicit staged order: DB schema → `fake_broker.py` options support → paper-trading extension (**each mechanism needs both a live-track and daily-track paper node, same shape as core's existing daily-track split — real gap caught during design review, needed to separate genuine logic bugs from ordinary live-tick-vs-backtest-Close price noise**) → paper/live-vs-backtest reconciliation (reuses the existing daily-track reconciliation pattern, replaying `scripts/stacked_model/`'s own backtest functions nightly) → Accountability Grid rows → truth-table permutation coverage → staged real-order live testing → edge-case hardening pass. Deliberately not started this session — real size of the undertaking, pick up fresh next time.
...
## [portfolio][backtest] Planned, not built, 2026-08-06 (very late) — Monte Carlo validation of the put-hedge design (part of candidate testing, after backtesting)
Follow-on to tonight's put-hedge deep dive (`docs/research_log.md`'s 2026-08-06 entries): backtesting against the 4 named historical crashes (2008 GFC, 2020 COVID, 2022 bear, 2000 dotcom) gave a real but narrow result — SOXL benefits from a 15-50% OTM put at every strike/every crash tested; KORU's extreme IV (234%) makes the hedge cost more than it saves in its two worst crashes (2008 GFC, 2022 bear) at every strike tested, though the loss shrinks going further OTM. Only 4 data points — real gap, not a distribution. **Design sketch**: (1) bootstrap many synthetic price paths from real historical returns (preserving vol clustering/fat tails, not a naive random walk) for SOXL/AGQ/KORU's underlyings, (2) run the real strategy kernel (entry/SL/TRAIL logic) against each simulated path to get a realistic trade sequence, (3) apply the put hedge (and separately, combined with skim-and-reserve) to each simulated sequence the same way tonight's real-crash test did, (4) aggregate across thousands of runs into a real percentile distribution (5th/50th/95th) of compounded return and max drawdown, hedged vs. unhedged vs. combined — answers "in what fraction of plausible futures does this help" instead of "did it help in these 4 specific things that already happened." Fits into the existing `docs/watchlist_candidate_checklist.md` candidate-testing pipeline (comparable in scope to the existing `sim_chaos_monkey.py` execution-adherence Monte Carlo, 1000 trials/node) — not a new process, the next stage of an existing one. Also still open from tonight, feeding into this: whether SOXL needs skim-and-reserve *in addition to* the now-validated put hedge (different jobs — put shrinks the loss, skim provides dry powder to redeploy — never tested combined), and the emergency-procedures runbook item above (real-time crash trigger still unvalidated regardless of which hedge design wins).
