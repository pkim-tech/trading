...
## [live-trading] Open, raised 2026-08-07 — should GDXU's node stay `state='live'` going forward, or revert to `paper` once the reconciled position closes?
Flipped to `live` tonight only to place a real protective stop for the orphaned position (see `deep_backlog.md`'s 2026-08-06/07 GDXU incident entry) — that doesn't itself decide whether GDXU should resume normal automated signal-scanning as a live node afterward, or go back to `paper` (its state since 2026-08-03) once flat. User's call, not decided.
