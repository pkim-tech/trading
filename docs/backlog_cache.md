...
## [live-trading] Found 2026-08-04 very late — three real bugs/design gaps found during a routine live-check, none fixed yet
(1) `signals_db.get_pending_buys_for_ticker_on_date` filters on `date(signal_time) = check_date` (exact match on today) -- wrong for `canary_overnight_carry` specifically, whose whole point is a pending buy that carries in from a PRIOR day; produces a false "no pending_buys row" for SDOW/DIA (both use this scenario) every day their resting order is older than today. Root cause of 2 of that day's 7 "unexplained" EOD deviations. (2) `scripts/status_check.py` reports `open_positions` only, never queries `pending_buys` -- same underlying gap, a still-resting trailing-buy shows misleadingly as "flat." (3) `active_signals.py`'s "outside signal window" status message hardcodes a stale time string (`"next: 10:25 or 14:55 ET"`) that doesn't match the real `_SIGNAL_WINDOWS` constant (`10:25-10:40, 15:25-15:40`) -- cosmetic, doesn't affect actual trading logic.
**Broader diagnosis**: (1) and (2) aren't coincidence -- multiple scripts (`status_check.py`, `coverage_check.py`, `watchlist_status.py`, `paper_trading_status.py`) each reimplement their own ad hoc view of overlapping state (open positions, pending buys, node config) instead of sharing one canonical "real current state" function, so the same class of bug can independently exist in each. Real fix candidates, not yet decided: (a) narrow — just fix the two confirmed bugs above; (b) broad — one shared `signals_db` function (e.g. `get_real_position_state(ticker, account=None)`) that all these scripts call instead of hand-rolling SQL. Either touches `signals_db.py`/coverage logic and needs the standard Opus review gate. See `.claude/skills/daily-routine-check/SKILL.md` for the full context.

## [backtest] Planned, not yet built, 2026-08-04 very late — SOXL drought-overlay parameter sweep (fixed_sl/arm_pct/trail_sell_pct as independent axes, cliff-safety checked, pooled across tickers for real sample size)
Full design: `docs/design.md`'s "SOXL drought-overlay parameter sweep" section. Depends on `scripts/drought_overlay_test.py`'s exit mechanics (already built tonight); needs a real grid search, not the coarse lockstep manual sweep done tonight.
...
Full design: `docs/design.md`'s "Put-hedge option-selection methodology" section. Depends on the SOXL sweep above landing on real risk parameters first.

## [live-trading][backtest] Planned, not yet built, 2026-08-04 very late — two-account paper trading (Account A: runs free, real market conditions; Account B: resets to backtest state + hourly-bar-close pricing, isolates price-availability as the only variable)
Full design, motivation, and implementation pieces: `docs/design.md`'s "Two-account paper trading" section (2026-08-04 very late addendum). Confirmed safe: paper trading's dedup is already `wl_id`-scoped, two nodes can share a ticker/account with no collision. Related to the still-open shared-state-function backlog item above — should be scoped together, not solved twice.
