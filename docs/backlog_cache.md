...
## [live-trading][security] Built 2026-08-09 (paper-only, staged real-order testing still open) — drought overlay/margin add-on/skim-and-reserve live automation
Schema/DB/paper-trading logic/daemon wiring/reconcile job/Accountability Grid/truth-table tests all built and paired-reviewed (put-hedge explicitly out of scope). Full detail: `docs/deep_backlog.md`'s 2026-08-09 entry. **Real live DB (`trading_live.db`) not yet migrated** — `ensure_tables()` only run against copies this session; the schema lands automatically next daemon restart. Next: staged real-order testing (needs organic signals over time, not a same-session step) and the edge-case hardening pass.
