...
## [live-trading] Found 2026-08-04 very late, (1)+(2) fixed 2026-08-05 — one cosmetic gap remains
(3) `active_signals.py`'s "outside signal window" status message hardcodes a stale time string (`"next: 10:25 or 14:55 ET"`) that doesn't match the real `_SIGNAL_WINDOWS` constant (`10:25-10:40, 15:25-15:40`) -- cosmetic, doesn't affect actual trading logic. Not yet fixed.
...
## [live-trading][backtest] Piece 4 still open, 2026-08-05 — `scripts/paper_vs_backtest_reconcile.py` needs a rebuild to use `wl_id`-scoped queries (now that `paper_trade_log` has one) and to report against `daily_track_reconciliation_log` instead of re-deriving its own comparison
Daily-track paper trading (hourly-close signal pricing, a nightly reconcile classification engine -- pure observation, no auto-resync/auto-halt, final design after the user reconsidered mid-session, node creation script) built 2026-08-05 -- see `docs/design.md`'s "Two-track paper trading" section and `backlog_resolved_recent.md`. Daily-track nodes not yet actually created against the live DB -- `scripts/add_daily_track_paper_nodes.py` is built and tested but hasn't been run for real; confirm with user before running.
