...
## Live/backtest parity gap — mostly resolved 2026-07-12, no broker fills needed
`TrailingBothZScoreBreakout`'s trailing-buy "wait for bounce" entry has no live-orchestration implementation (hands off to a broker-side trailing-buy order) — `scripts/verify_live_parity.py` deliberately can't compare it. Instead of waiting on real broker fill data, built `scripts/verify_trailing_buy_resolution.py`: re-detects every recent signal's bounce-entry using yfinance 5-min bars (real intra-hour tracking) and diffs against what the hourly-bar kernel (`_simulate_trail_both`) would catch for the same signal. Result across all 11 watchlist-9 tickers (134/138 signals matched, last ~58d): mean price diff +0.36%, and 5/11 tickers within noise of 0%. **SOXL is a real outlier**: +1.81% mean fill-price penalty (individual signals up to +7.5%), driven by its `trail_buy_pct=1%` being far tighter than its own ~3.56% median intra-hour swing (ratio 3.57) — volatile enough to cross/re-cross the trigger within an hour, so 5-min tracking locks in an earlier/worse fill than the hourly kernel models. TQQQ/NUGT (ratio 1.5-1.75) show smaller +0.37-0.84% drift; everything else (ratio <1.3) is at parity. Formalized as a repeatable procedure in `docs/watchlist_candidate_checklist.md`. **Not fully closed**: this validates the *entry-price* assumption is broadly sound (except SOXL); it doesn't validate the broker's trailing-buy order mechanics themselves (whether Schwab's own trigger/fill logic matches the running-low model at all) — that piece still has no real-fill-time-vs-signal-time verification (see also the fill-price/drift accuracy item above).
