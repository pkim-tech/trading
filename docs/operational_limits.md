...
| 2 | Below z trigger, in window → BUY trailing order alert | `notify_buy_signal` → `_build_buy_blocks`: trigger price, shares, notional (~$50k), max-vol cap, account | Bar-close, in-window only | `active_signals.py:967-973` | OK — account now shown on every BUY alert unconditionally (fixed a prior 2026-07-09 session, doc was stale) |
| 3 | Trailing buy pending → "should have filled by now" reminder | Doesn't exist. "Executed" button lets you self-report a fill any time, but nothing nags like the sell-side reminder does | — | — | **Gap** — matches the open backlog item "Trailing-buy fill confirmation" (2026-07-07), still never built |
