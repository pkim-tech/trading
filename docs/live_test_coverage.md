...
| Post-fill top-up places a real order | `_reconcile_fill` | Unit test (`test_part3_gap_resize.py`); `scripts/live_sim_harness.py::scenario_reconcile_fill_topup` (2026-07-23, forced shortfall); `fake_venue/scenarios_post_fill_topup.py` via `scripts/fake_venue_harness.py --scenario post_fill_topup` / `tests/test_fake_venue_post_fill_topup_scenario.py` (2026-08-16, Phase 2 — drives a REAL second broker order through the stream fill path, not just the top-up placement itself) | **Done** | This row was stale (predated the real fix) — **updated 2026-08-16, correcting it against `scripts/coverage_registry.py`, the current live source of truth for this scenario's status (see CLAUDE.md's Key Files note)**: real successful top-up observed live 2026-08-10 (RETL/`soxl_ira`, `result='placed'`, closing the `daily_order_cap`-starvation risk this row used to describe as open). Same-session (2026-08-16) paired-review fix, `_reconcile_fill`: the top-up leg's `entry_price` used to be recorded as the ORIGINAL fill's `fill_price`, not the top-up order's own actual (confirmed) execution price — fixed via a short synchronous poll for non-gap-correction top-ups (the `check_gap_resize` pre-open path skips the poll by design, since that order structurally can't have filled yet and the poll would otherwise delay real SL placement by up to ~15s for zero benefit — see `_reconcile_fill`'s docstring). `coverage_events['top_up']`'s detail now carries `confirmed=True/False` so an approximated vs. broker-confirmed price is distinguishable after the fact. |
...
| A hold-time-forced exit (armed, hold time expires before any real trail-stop breach) force-replaces the stale resting trail order with a real market sell, instead of reusing it forever | `signals_notify._attempt_automated_exit_sell` (`still_unreplaced_trail_order` check, 2026-07-30) | `test_fake_broker_sh_scenario.py::test_armed_position_past_max_hold_gets_forced_market_exit` — **passes but doesn't exercise the actual bug** (no pre-existing `exit_pending` seeded, so the reuse-early-return this fix targets is never reached); needs rewriting to seed `exit_pending.order_id == exit_order_id` first | Not started (live retest scheduled) | Real incident 2026-07-30: SH stuck in `TRAIL`/`exit_pending` for days — the 2026-07-29 fix (`exit_forced_by_hold_time`) only ever worked the *first* time hold-time expired on a position with no `exit_pending` yet; the reuse-guard added 2026-07-27 short-circuited before it on every subsequent poll. Fixed same session (scoped to zero-effect on TP/SL/TIME reasons and genuine TRAIL breaches — see `docs/deep_backlog.md`'s 2026-07-30 evening entry). SH's `open_positions.max_hold_hours` bumped 31→38 and `exit_forced_by_hold_time`/`exit_pending` cleared so the retest lands ~2026-07-31 11:30 ET during market hours, not overnight |
| Full auto-fill-detection pipeline (real BUY fill detected on an order we didn't place through the daemon's own signal flow → position opens → real protective SL placed) | `signals_notify.check_auto_fills` → `_reconcile_buy_fill` → `_place_stop_loss_for_position` | None yet for this exact path (manual real fill, not daemon-placed) | Not started | Staged live 2026-07-30: LABD (node 152, scenario "not armed, SL sell") and ERY (node 153, scenario "armed, genuine trail breach") both bought manually (1 share each, real fills, order_ids on file), `pending_buys` rows created pointing at the real order_ids, both tickers added to `SCHWAB_AUTOMATION_TICKERS`, both ticker+node auto-fill-detection flags enabled. Needs the daemon restarted with a fresh token (refresh token expired same session, real reauth required) before this can actually fire |
| A trailing-buy that never bounces is abandoned (real `cancel_order` call, or local row cleared if nothing real was ever placed) once `max_hold_hours` elapses, instead of resting forever and blocking every other BUY in that account | `signals_notify.check_entry_abandon`; `paper_trading.update_paper_buys` (paper leg) | 12 unit tests (`tests/test_entry_abandon.py`), incl. the full `(dry_run, order_placed, order_id)` truth table and a raced-fill race | Not started | Built 2026-07-31 (was the [HIGHEST] item in that session's exit/arm/entry audit) — first-ever production caller of `schwab_client.cancel_order`, a real broker mutation, never yet observed against a genuinely resting real (non-dry_run) order. Also registered in `scripts/coverage_registry.py` (`entry_abandon_timeout`). A first version of this had a real-money bug (found by independent review before landing, not live): the manual "Trailing Buy Order Placed" Slack flow leaves `order_id=NULL`, which the first version treated as "safe to clear + claim cancelled" — could have orphaned a real resting order with zero local tracking. Fixed to alert and leave the row untouched for that specific case instead. |
