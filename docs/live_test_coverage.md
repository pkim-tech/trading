...
| Post-fill top-up places a real order | `_reconcile_fill` | Unit test (`test_part3_gap_resize.py`); `scripts/live_sim_harness.py::scenario_reconcile_fill_topup` (2026-07-23, forced shortfall) | Not started | Real-money risk if untested live: this is the path with the phantom-shares bug fixed this session |
