...
**Revised fix direction (2026-07-04, user confirmed via Schwab)**: no polling state machine needed — Schwab supports trailing-stop-**buy** orders (confirmed by the user: reference price ratchets down with a falling low, e.g. $30→$20, and triggers on a bounce off the running low, e.g. $20.60 not the original $30 basis). This is the exact `running_low`/`buy_trigger` mechanic `_simulate_trail_buy` models. Live execution becomes: at bar close when the z-score signal fires, convert the resting staged order (today's "absurdly low limit" placeholder) directly into a trailing-stop-buy order at `trail_buy_pct`% — same single edit-and-submit action as the existing limit→market swap for v1.5/v2.8 — and let the broker's own trailing engine track the low from that point on. This closes the reaction-time gap for free (the broker starts tracking from order-conversion time, which is the same bar-close moment the kernel seeds `running_low`), and needs no persisted `pending_entries` state or extra polling logic.

Still to do: add `_STRATEGY_LABELS` entries for both classes with this action text; verify Schwab's trailing-buy ticket accepts the needed trail_buy_pct values (some brokers cap/round trailing percentages) before trusting it with real money; run a live-parity sanity check once an actual v1.9/v1.10 trade has gone through, since the broker's tick-level trail tracking may not match the kernel's bar-level low exactly.
