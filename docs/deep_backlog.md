...
**Fix 1 — `run_optimization_sweep.py`**: `sweep_runs` gained `data_start`/`data_end` (real min/max Datetime across each run's tickers' cached hourly CSVs, read via new `_tickers_data_date_range()` at `start_sweep_run()` time) — every future sweep/sub-sweep run now records the data window it actually loaded, the "what data did this run see" counterpart to the existing `git_commit`/`kernel_dirty` ("what code ran it"). `data_start` backfilled across all 1,128 existing `sweep_runs` rows (safe: a ticker's earliest cached bar practically never moves — no split-guard rescale ever truncates from the front, and yfinance's hourly window doesn't reach back further over time). `data_end` deliberately **not** backfilled for historical rows — the actual CSV content at each past run's moment is genuinely gone, so fabricating a plausible value from today's CSV would be worse than `NULL` (which honestly says "unknown"). One real bug caught during implementation: the first version of `_tickers_data_date_range()` used `df.empty` to check for no data after `pd.read_csv(..., usecols=[0])` — `df.empty` is `True` whenever there are zero *columns*, not just zero rows, and `usecols=[0]` intentionally leaves zero data columns (only the index) — every real ticker was silently returning `(None, None)`. Fixed to check `len(df.index) == 0` instead; verified against JNUG/AGQ (real dates) and a nonexistent ticker (clean `None, None`, no crash).
