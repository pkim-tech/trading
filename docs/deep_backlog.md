...
## [backtest][live-trading] Partially built 2026-08-11 (late) — `watch_list_candidate_link` table shipped after a trust-breaking session of wrong param-matching claims; 13 real nodes traced, 3 new $10k `ira` positions added (GDXU/KORU/DFEN), SOXL's missing drought overlay fixed; sweep-campaign-level traceability still open
...
**Still open**: sweep-campaign-level provenance (tying a `candidate_nodes` row back to a real, identifiable backtest sweep run — `backtest_cache.run_timestamp`/`version`, not an invented "list run" concept) — design direction agreed, not built, needs its own design pass against the real sweep tooling. HIBL still fully unresolved: no record of which candidate its `soxl_ira` live node (id=154) should actually be running — user's recollection (unverified): candidate id=113 (not id=111, which is what the live node's params actually match), from "the v5-overlay sweep." SH's `soxl_ira` node still has `trail_buy_pct` widened 1%→5% from the resolved `post_fill_topup` wait (separate, already-open backlog item, not touched this session). Real `.env`/2 uncommitted pre-session files (`scripts/sim_wash_sale_impact.py`, `scripts/sim_real_portfolio_rebalance.py`) were left untouched — predate this session, never investigated.
