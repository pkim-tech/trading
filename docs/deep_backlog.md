...
User's call: the ~11 real live tickers is the right scope, the broader `trading_universe.db` research-side trim was never actually wanted — the item's "still open if" condition never applied. EDC is deliberately excluded (not an oversight — its `watch_list` node was fully removed 2026-07-19, one open manual position hand-tracked via spreadsheet separately, per the EDC pilot idea item still open below).
