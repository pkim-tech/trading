...
**Real capital, but no urgency to fix tonight**: confirmed zero open positions and zero pending buys on all 3 `ira` nodes (`cache/live/trading_live.db` checked directly) -- nothing to unwind, this is a clean config-only situation. **Blocked on**: a concurrent session was actively wiring in a new Roth account ("Roth-A-1" per the user, not yet in `schwab_safety.ACCOUNTS`) at the same time this was found -- status/completion unknown as of this entry. Once that account exists in the codebase, GDXU/DFEN/KORU's `ira` nodes should move there (exact mechanism TBD -- `signals_db.py` has no "retire" state, only `remove_node()` (hard delete) or `set_node_state(..., 'paper')` (reversible demotion); which one applies depends on whether the intent is "recreate fresh in the new account" or "migrate this exact row").
