...
- **Dispatch overhead optimization**: Profiled 2026-07-03 (see `docs/dispatch_telemetry_results.md`). Worker-level cache dropped per-node kernel time from ~279ms to ~7.2ms median, but realized dispatch speedup is only 3-5x overall because result collection (IPC/pickling return values) is 88% of dispatch overhead (~4.4s per 2000 nodes). Kernel time now ~2% of per-node dispatch cost. Next optimization: batch DB inserts (currently one INSERT per result row) or async result collection. Batching at `ProcessPoolExecutor.submit()` level wouldn't help — the bottleneck is downstream (IPC result return + SQLite write).
