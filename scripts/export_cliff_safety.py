...
def load_cliff_safety(conn, version_strategy_pairs):
    ...
    for version, strategy in version_strategy_pairs:
        ...
        for ticker in tickers:
            ...
            # Multiple stop_loss/entry_timing campaigns can share one version string
            # (v4+) — keep the neighbor box inside the same campaign as the best row.
...
if __name__ == '__main__':
    ...
    with sqlite3.connect(DB_PATH, timeout=60) as conn:
        ...
        ).fetchall() if not v.startswith(('v1.', 'v2.'))]
